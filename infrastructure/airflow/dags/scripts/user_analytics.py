...
def process_user_analytics(*args):
    """Process clicks data to generate user analytics"""
    ...
    INSERT_QUERY = f"""SELECT
                            ...
                            SUM(CASE WHEN duration > 0 AND EXTRACT(HOUR FROM datetime) BETWEEN 6 AND 13 THEN 1 ELSE 0 END ) as morning_hour_views,
                            SUM(CASE WHEN duration > 0 AND EXTRACT(HOUR FROM datetime) BETWEEN 14 AND 21 THEN 1 ELSE 0 END ) as evening_hour_views,
                            SUM(CASE WHEN duration > 0 AND (EXTRACT(HOUR FROM datetime) >= 22 OR EXTRACT(HOUR FROM datetime)  <=  5) THEN 1 ELSE 0 END ) as night_hour_views,
                            event_date
...
#     process_user_analytics('2017-01-04')
